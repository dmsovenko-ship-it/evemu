...
| **Alliance job completion** — проверка альянсовых ролей для завершения джобов (было заглушка) | ❌ | ✅ |
| **Invention install validation** — чтение и валидация inventionItems данных при установке (было закомментировано) | ❌ | ✅ |
| **Blueprint spawn copy/orig** — явное указание copy/runs при создании чертежа через generic spawn | ❌ | ✅ |
| **POS assembly lines** — автоматическое создание entry в ramAssemblyLines при запросе линий у POS-структур | ❌ | ✅ |
| **POS ItemLocationCheck** — вместо CustomError проверка, что контейнер — валидная POS структура | ❌ | ✅ |
