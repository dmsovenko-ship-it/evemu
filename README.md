...
| **POS defence & industry** — EWAR batteries (energy neutralizer / sensor damper / ECM), guaranteed warp-in guards, force-field barrier, Ship Maintenance Array (store/board/scoop/refit), Assembly Array manufacturing, hardener recalculation, corp-only structure storage | **Оборона и индустрия POS** — EWAR-батареи (нейтрализатор / дампер / ECM), гарантированный прилёт гвардов, барьер поля, SMA (склад/перефит кораблей), производство на Assembly, пересчёт харденеров, доступ к хранилищу структур только для корпа |
