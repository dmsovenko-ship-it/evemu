"""Extract and decompile starMapSvc.py and route.py from compiled.code"""
...
targets = {
    'script:/ui/shared/maps/starMapSvc.py': 'starMapSvc',
    'script:/ui/shared/maps/route.py': 'route',
}
...
for (filename, type_val), (encrypted_code, hash_val) in codes:
    ...
    # Fresh provider and key for each entry
    hProv = HCRYPTPROV()
    ...
    dec_buf = ctypes.create_string_buffer(encrypted_code)
    dwOut = ctypes.c_uint32(len(encrypted_code))
    advapi32.CryptDecrypt(hTdesKey, HCRYPTHASH(0), 1, 0, dec_buf, ctypes.byref(dwOut))
    dec = zlib.decompress(dec_buf.raw[:dwOut.value])
