...
protected:
    ...
    // locationType arrives as a Unicode string from the client ('dungeon'/
    // 'agenthomebase' etc — see eveBaseLink GetBookmark); locationNumber as int.
    PyResult WarpToLocation(PyCallArgs& call, PyWString* locationType, PyInt* locationNumber, PyFloat* warpRange, PyBool* fleet, PyInt* referringAgentID);
