...
PyResult IndexManager::GetDevelopmentIndicesForSystem(PyCallArgs& call, PyInt* solarSystemID) {
    ...
    PySetFieldRelease(row1, "attributeID", new PyInt(EveAttrEnum::AttrdevIndexSovereignty));
    PySetFieldRelease(row1, "points", new PyInt(sovPoints));
    PySetFieldRelease(row1, "increasing", new PyBool(sovPoints < 30));

    // Military (1 per day, capped at 10)
    uint32 milPoints = std::min<uint32>(uint32(daysSinceClaim), 10);
    PySetFieldRelease(row2, "attributeID", new PyInt(EveAttrEnum::AttrdevIndexMilitary));
    PySetFieldRelease(row2, "points", new PyInt(milPoints));
    PySetFieldRelease(row2, "increasing", new PyBool(milPoints < 10));

    // Industrial (1 per day, capped at 10)
    uint32 indPoints = std::min<uint32>(uint32(daysSinceClaim), 10);
    PySetFieldRelease(row3, "attributeID", new PyInt(EveAttrEnum::AttrdevIndexIndustrial));
    PySetFieldRelease(row3, "points", new PyInt(indPoints));
