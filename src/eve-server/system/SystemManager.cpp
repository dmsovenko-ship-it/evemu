...
uint32 SystemManager::GetRandBeltID()
{
    if (m_beltVector.empty())
        return 0;
    return m_beltVector[MakeRandomInt(0, m_beltVector.size() - 1)];
}
...
void SystemManager::MakeSetState(const SystemBubble* pBubble,  SetState& into) const {
    ...
    PyPackedRow* row = new PyPackedRow(sDataMgr.CreateHeader());
        PySetFieldRelease(row, "itemID", new PyLong(m_data.systemID));
        row->SetField("typeID",        PyStatic.NewInt(5));
        row->SetField("ownerID",       PyStatic.NewOne());  // should this be owning factionID?  yes
        PySetFieldRelease(row, "locationID", new PyInt(m_data.constellationID));
        row->SetField("flagID",        PyStatic.NewZero());
        row->SetField("quantity",      PyStatic.NewNegOne());
        row->SetField("groupID",       PyStatic.NewInt(5));
        row->SetField("categoryID",    PyStatic.NewInt(2));
        PySetFieldRelease(row, "customInfo", new PyString(""));
    into.solItem = row;
...
void SystemManager::ProcessGhostShips() {
    ...
    while (it != m_ghostShips.end()) {
        ...
        // Emergency warp: wait for timer, then initiate and monitor warp
        if (emergencyWarp and pShip->DestinyMgr() != nullptr) {
            if (now < expireTime) {
                ++it;  // initial 60s timer hasn't expired yet
                continue;
            }
            if (pShip->DestinyMgr()->IsWarping()) {
                ++it;  // warp in progress — wait
                continue;
            }
            // Ship not warping — check if warp completed or needs initiating
            if (pShip->DestinyMgr()->GetState() == Destiny::Ball::Mode::STOP
                and pShip->DestinyMgr()->GetSpeedFraction() < 0.01f) {
                // Ship is stationary after warp (or never moved) — remove
        ...
        // Normal expiry (safe logoff or combat logoff): remove if time is up
        if (now >= expireTime) {
