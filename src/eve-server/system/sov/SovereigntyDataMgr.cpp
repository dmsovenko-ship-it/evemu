...
PyRep *SovereigntyDataMgr::GetCurrentSovData(uint32 locationID)
{
    ...
    if (IsConstellationID(locationID))
    {
        ...
        // Add faction (empire) systems in this constellation not covered by sov data
        DBQueryResult fRes;
        if (sDatabase.RunQuery(fRes,
            "SELECT mss.solarSystemID, mss.factionID"
    ...
    else if (sDataMgr.IsSolarSystem(locationID))
    {
        ...
        // Add faction systems in this region for single-system query
        SystemData sysData;
