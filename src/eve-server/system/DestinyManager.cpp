...
void DestinyManager::WarpDecel(uint32 sec_into_warp) {
    ...
    WarpUpdate(v);
    // Fire WarpStop once the ship is at the target, but hold first so the
...
void DestinyManager::WarpStop(double currentShipSpeed) {
    ...
    // client-ball at its own convergence point — no visible end-of-warp teleport.
...
void DestinyManager::SendJumpInEffect(std::string JumpEffect) const {
    std::vector<PyTuple*> updates;
    // Use OnSpecialFX10 (no graphicInfo field) to avoid Crucible fxsequencer crash.
    OnSpecialFX10 sfx;
        sfx.entityID = mySE->GetID();
        sfx.moduleID = mySE->GetID();
        sfx.moduleTypeID = mySE->GetSelf()->typeID();
        sfx.targetID = mySE->GetID();
        sfx.chargeTypeID = 0;
        sfx.area = new PyList();
        sfx.guid = JumpEffect;
        sfx.isOffensive = 0;
