...
PyResult MissionMgrService::GetMyCourierMissions(PyCallArgs& call)
{
    ...
    if (!sDatabase.RunQuery(res,
        ...
        "  o.rewardISK,"
        "  o.rewardLP,"
        "  CASE WHEN o.originID > 0 THEN o.originID ELSE (SELECT stationID FROM agtAgents WHERE agentID = o.agentID) END AS originID,"
        "  CASE WHEN o.originOwnerID > 0 THEN o.originOwnerID ELSE (SELECT corporationID FROM agtAgents WHERE agentID = o.agentID) END AS originOwnerID,"
        ...
        "  CASE WHEN o.destinationID > 0 THEN o.destinationID ELSE (SELECT stationID FROM agtAgents WHERE agentID = o.agentID) END AS destinationID,"
        "  CASE WHEN o.destinationTypeID > 0 THEN o.destinationTypeID ELSE 0 END AS destinationTypeID,"
        "  CASE WHEN o.destinationOwnerID > 0 THEN o.destinationOwnerID ELSE (SELECT corporationID FROM agtAgents WHERE agentID = o.agentID) END AS destinationOwnerID,"
