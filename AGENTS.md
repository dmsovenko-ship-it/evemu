...
## Current State
Session saved. Server on remote host `172.20.1.47`, SSH user: `dmitry` (password `gbnjy78`), path: `/opt/evemu`. Всё ниже задеплоено и ПЕРЕСОБРАНО на сервере (docker-compose build + up, контейнеры под управлением compose). Коммиты в origin/master: `abdeb8fe`...`b4f424b9` (см. секции ниже).
...
- **✅ Фикс промахов файтер-бомберов (`74221cb1`)**: юзер «лог боя пишет далекий промах, хотя я вплотную». КОРЕНЬ: файтер-бомберы (группа 1023, SDE-холлы Cyclops/Malleus/...) — AoE-боеприпасы: имеют `AttrEntityAttackRange`=20000, `thermalDamage`=3000, `aoeCloudSize`/`proximityRange`, но НЕ имеют `AttrTrackingSpeed`/`AttrFalloff`/`AttrOptimalSigRadius` (это не турели). `GetDroneToHit` делил на ноль → toHit=0 → combat log «misses completely (too far away)» на каждый выстрел бомбера, независимо от дистанции. Фикс: `FighterBomberAttack` применяет toHit=1.0 (бомбы взрываются при подлёте — always hit). Обычные файтеры/дроны (tracking/falloff есть) сохраняют турельную формулу.
...
- **Осталось (Этап 0.2 прогон)**: ASAN-сервер запущен на 26000/26001 (UBSAN halt_on_error=1, ASAN detect_leaks=0 abort_on_error=1). Юзер играет боевые сценарии (дрон-пуск, бой с ботами, ТКУ, варп), ASAN пишет трейсы в docker logs → лечить адресно. После прогона вернуть обычный образ (`docker rm -f server` + `docker-compose up -d`).
...
- Крупные системы (не реализованы): Memory Mgmt/RefPtr→shared_ptr (~400 файлов, полный перевод НЕ реалистичен — аудит 27 авг: 4281 new Py*, 6443 raw Py*, ~6000 мест; вместо него Этап 0: укрепление RefObject `b4f424b9` + ASAN-аудит), PyRep leak-фиксы. Джамп-драйвы/бриджи — РЕШЕНО `6dd25d3d` (валидация активного цино + миграция топлива по расам). Клоны — РЕШЕНО `015f5736`+`ffc27824` (AcceptShipCloneInstallation сломанный SQL + CloneJump не двигал игрока). PI — РЕАЛИЗОВАНО (Colony/PlanetMgr/PlanetDB, PIEnabled=true), нужен e2e-тест. FW Plex capture — РЕАЛИЗОВАНО `b62e1310` (плексы спавнятся, таймеры захвата с лимитами по кораблю, contested-пауза, NPC-защитники, LP, переворот системы по occupierID).
...
3. **Мобилка после анчоринга не скрамблит** — ЗАКРЫТО `d8ace19c` (25 авг): MWD сразу online после анчора (bubble + scramble активны), стоп-стак деплоябла `eb08f76c` (Split(1)). Проверить после пересборки.
