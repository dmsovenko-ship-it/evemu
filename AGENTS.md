...
- Крупные системы (не реализованы): FW Plex capture, Memory Mgmt/RefPtr→shared_ptr (~400 файлов), PyRep leak-фиксы. Джамп-драйвы/бриджи — РЕШЕНО `6dd25d3d` (валидация активного цино + миграция топлива по расам). Клоны — РЕШЕНО `015f5736`+`ffc27824` (AcceptShipCloneInstallation сломанный SQL + CloneJump не двигал игрока). PI — РЕАЛИЗОВАНО (Colony/PlanetMgr/PlanetDB, PIEnabled=true), нужен e2e-тест.
