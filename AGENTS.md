...
- **Осталось (Этап 0.2)**: ASAN-сборка тестового образа (`docker build` + флаги `-fsanitize=address,undefined` и `REFPTR_HARD_FAIL`) → прогнать боевые сценарии → зафиксировать базовую линию UAF → лечить адресно.
