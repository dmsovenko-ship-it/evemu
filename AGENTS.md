...
- **✅ Этап 0.2 (ASAN) — сборка готова + найден реальный баг**: Dockerfile получил build-arg `CMAKE_CXX_FLAGS_EXTRA` (`2d9a4acf`) для прокидывания `-fsanitize=address,undefined -DREFPTR_HARD_FAIL -fno-omit-frame-pointer -O1 -g`. Образ `evemu_server_asan` собран (бинарь 1 ГБ, libasan.so.8). ⚠️ Первая сборка падала DEADLYSIGNAL — это OOM от `make -j$(nproc)` (ASAN-компиляция ~1-2 ГБ/процесс), повторная сборка проходит. **🔴 Баг, найденный ASAN (фикс `9345edd1`)**: `XMLParser::ElementParser` имеет виртуальный `Parse()`, но НЕ виртуальный деструктор → `ClearParsers` удаляет производные парсеры через базовый указатель (`Generator` 32 байта как `ElementParser` 8 байт) → new-delete-type-mismatch, порча кучи при каждом запуске генератора пакетов (eve-xmlpktgen). Добавлен `virtual ~ElementParser()`. ASAN окупился на первом же запуске сборки.
- **Осталось (Этап 0.2 прогон)**: поднять ASAN-сервер на портах 26000/26001 вместо обычного (`docker rm -f server` + `docker run` с `-e ASAN_OPTIONS=detect_leaks=0:abort_on_error=1 -e UBSAN_OPTIONS=print_stacktrace=1`), юзер играет боевые сценарии (бой с ботами, ТКУ, дроны, варп), ASAN пишет трейсы в docker logs → лечить адресно. После прогона вернуть обычный образ (`docker-compose up -d`).
