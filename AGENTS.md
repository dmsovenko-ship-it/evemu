...
- **Память — что осталось на потом**: Missile wrapper ~200Б/пуск (нужен реестр владельцев как `EntityList::m_probes`), ASAN-генератор (собирать eve-core дважды: ASAN + plain), мониторинг REFPTR__ERROR/REFPTR_HARD_FAIL на живом, core_pattern вернуть в apport после следующего пойманного краша. Нижележащая дыра (деструкторы контейнеров не освобождают items) остаётся by-design.
