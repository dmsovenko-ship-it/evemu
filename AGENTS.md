...
HEAD origin/master: `5afdbeeb`. Все изменения закоммичены/запушены. **Требуется применить миграции** `20260910000000-bot_pi_colonies.sql` и `20260910000001-interbus_customs_offices.sql`.
- **Кап ботов починен**: reuse был завязан на отдельный `COUNT` (`if (poolCount>0)`), под нагрузкой count мог вернуть 0/ошибку → спавнер минтовал новых чаров (пул 2740 при капе 2500). Теперь reuse пробуется ВСЕГДА; COUNT — только для пустого пула (при ошибке считаем кап достигнут); кап перепроверяется на ретраях топ-апа. `TrimPilotPool()` на старте обрезает до капа.
- **Инкурсии**: сайт получала только первая инкурсия (`Check(false)` внутри цикла гасил таймер) — вынесено `spawnTick=Check(true)`; `SpawnSites` бутит только системы с онлайн-игроками (без `FindOrBootSystem` всего созвездия). Белые квадраты+0 урона: инкурсийные NPC были заглушками 2190-2192 (группы 1052-1056, без дамаг-атрибутов) → заменены на реальных Sansha (группы 565/566/567). Фракция форсится в Sansha. Волновые карманы декорируются, спавны разбросаны по кольцу, второй/третий карман с декором.
- **Лут/заряды**: боты-туретчики носят заряды по `chargeGroup1` (кристаллы/патроны) + Nanite Repair Paste при Thermodynamics; lossmail-выпадение: одностековые модули теперь 50% (были всегда destroyed), риги/саб всегда destroyed; **выпавшее материализуется во врек** (`PlayerBot::MoveDroppedItemsToWreck`, 50% бросок из in-memory инвентаря, как у игроков).
- **Профессия Industrialist** (ролл ~7%, industrial hull): полная рекурсивная производственная цепочка по `invTypeMaterials` (покупка недостающего `BotBuyStock` → импорт из региона `BotBuyStockRemote`), POS у луны (Control Tower + Assembly Array + Silo + орудийные батареи + web-батарея), крафт физически в Assembly Array с перевозом на станцию, PI-цепочки P1→P4 по `schematicsTypeMap` (колония в `botColonies`), свой Customs Office на планете колонии. Логистика: курьерка на хаб/продажа.
- **POS-оборона**: `POS_AI` обобщён на `StructureSE` → работает и у `BatterySE` (web/scram/neut) и у `WeaponSE` (урон, расход зарядов chargeGroup1). Наведение учитывает настройки башни: стендинг, security-status, война, + хайсек-гейт (криминал/агрессор/аутло). **Ручное управление** (Crucible): `AssumeStructureControl` требует Starbase Defense Management + 15 км и обездвиживает; `dogmaIM.AddTargetOBO/RemoveTargetOBO` задают ручную цель, `POS_AI` приоритизирует её (фокус, полная дальность). **Боты-охранники** (2-3 Hunter того же корпа) орбитят башню, ассистят ручную цель; появление: login-at-station→варп (хайсек) или login-at-POS (нули, 50/50 в хайсеке).
- **Кастом-офисы**: фикс запуска — орбиталка биндится к ближайшей планете (`customInfo=planetID`), иначе `planetID=0` и анкор невозможен (в т.ч. в империи). Миграция сидит NPC InterBus-офисы (typeID 4318, corp 1000148) на ВСЕХ хайсек-планетах (~9339). Боты-производственники анкорят свой CO на планете колонии.
- Правки в портале (репо evemu-portal): разнесение конфига (`config.php` user / `const.php` engine / `config.sample.php`, `config.php` в .gitignore), оформление `/mail`, полный маппинг Notify::Types, правила, SMTP/2FA — см. AGENTS evemu-portal.
