...
## 17 сент.: клиент завис (AppHang) — диагностика настроена, причина не подтверждена
- **Факт**: 17 сент 10:55:2x MSK клиент ExeFile.exe (Crucible 7.30.35) завис в космосе (Mr Tort, Laic, над станцией, БЕЗ боя, БЕЗ /tidi — TiDi-код на сервере ещё не был задеплоен). Сервер увидел мёртвый сокет в 07:55:27 UTC → unsafe logoff + emergency warp (корабль цел, emergency warp отработал). WER: AppHang 1002 в 10:55:40, **дампа нет** (hang type 0x08000000, стека в отчёте нет). EventLog — только 1001/1002.
- **Ловушка установлена**: WER LocalDumps для ExeFile.exe в **HKCU** (HKLM недоступен без админки — HKCU-вариант работает): DumpFolder=C:\Dumps, DumpType=2 (full), DumpCount=3. **При следующем зависании/падении — полный дамп в C:\Dumps, разобрать стек** (cdb/windbg).
- Сервер в момент зависания: mass churn 27 систем (unload→boot с кастомс-NPC каждые ~5 мин — цикл «inbound spawn бутит систему→пустеет→unload»), ошибок и пакетных штормов в логе нет. Клиентские логи не ведутся (.lbw от июля), dev-консоль отключена.
- Сервер запущен 16 сент 14:48 UTC — **без** фиксов f5e69ecd (tower link self-heal), 6b308ee5 (боты в локале), TiDi d7420612, 068e42cb (миграция idempotent). Пересборка+рестарт закатывают всё.

## 16 сент. (ночь, часть 2): бот-модули POS теряли башню + пустой локал после оптимизации (`f5e69ecd`, `6b308ee5`)
- **«StructureSE::Init ... is invalid. why are we here?» спамом на буте**: все бот-модули POS сохранялись в `posStructureData` с **`towerID = 0`** (деплой писал модуль до появления связи с башней). При загрузке `Init` искал башню через `GetSE(0)` → null → «invalid» и модуль не восстанавливал связь (без AddModule — без POS_AI/учёта PG-CPU). При этом защита поля работала (от состояния башни, не от поля) — поэтому «лочить не даёт». Фикс: (1) `Init` m_module — если towerID не резолвится, **самолечение по близости** (башня ≤100 км, линк в БД, инициализация продолжается) — покрывает все битые строки без миграции; (2) `StructureSE::SetBotTower(towerID)` + вызов в `DeployBotPOS` для каждого модуля — связь персистится при деплое. Мусорная строка itemID=0 из posTowerData удалена на живой.
- **«Не вижу челоботов совсем в локале» после оптимизации**: мой глобальный троттлинг спавна стоял **до** пер-системного кулдауна → первая по systemID система (Джита) брала слот каждые 6с, попадала в свой 15с кулдаун и **сжигала его вхолостую** — остальные системы (включая игрока) голодали вечно, ушедшие боты не замещались. Фикс: (1) пер-системный гейт проверяется **первым** — кулдаун-система выходит, не расходуя глобальный слот; (2) `BotMgr::Process` — **два прохода**: системы с игроками заполняются первыми, always-on хабы берут остаток; (3) тайминги: глобально 6с→4с, пер-система 15с→10с (планка игрока набирается за ~5 мин).
- Проверка: рестарт без спама «why are we here?» (один раз `tower link healed`), боты в локале игрока с первых минут (планка за ~5 мин), модули POS связаны с башней и стреляют.

## ❌ Отклонено (не делать, не предлагать повторно)
- **Системный ревоунершип PyRep→intrusive_ptr** (~4000 аллокаций / ~1520 steal-границ / 544 ручных рефов): 3-6 недель + недели отладки, каждая ошибка = UAF-краш в бою. Статус-кво (audit-driven точечные фиксы + DeepClearRep для кеш-пути + malloc_trim) даёт ~95% пользы за ~1% риска.
...
Под планируемую зарубу. **Вынос региона на отдельное ядро — ОТКЛОНЁН** (десятки синглтонов без локов; это переписывание архитектуры, не фича). Вместо него — TiDi:
- **Состояние** в `SystemManager`: `m_timeScale` (1.0/0.5/0.25/0.1), `SetTimeScale(level)` — перерескаляет DestinyMgr каждого тик-энтити и шлёт пилотам нотификацию «Local time dilation: X%».
- **DestinyManager::ApplyDilation(newScale)**: ratio-рескейл `m_maxShipSpeed/m_maxSpeed/m_velocity/m_speedToLeaveWarp` + рассылка **`SetBallSpeed`** — это официальный TiDi-механизм, обновляет max speed в КЛИЕНТСКОЙ симуляции для любого типа шара (клиент замедляется вместе с сервером). ⚠️ НЕ использовать `SetSpeedFraction` для реброадкаста — может выкинуть STOP-корабль в GOTO.
- **`SetMaxVelocity`/`SpeedBoost`**: эффективная скорость = raw × dilation (сырые атрибуты не трогаем).
- **Таймеры**: `ActiveModule::SetTimer` (все циклы модулей, ×factor на новом цикле), `NPCAIMgr` ctor (attack cycle), Missile (flight/hit timers, `SetHitTimer`).
- **Не замедляется** (как на оффе): доки, скиллы, продакшн, кошельки.
- **Команда**: `/tidi off|50|25|10` (Role GMH, в текущей системе), гейт `<TiDiEnabled>` в `<world>` (дефолт false). Репо-конфиг обновлён; на живой конфиг добавить тег вручную при включении.
- **Проверка в бою**: вкл 10% → корабли ползут в ~10× медленнее, клиент согласован (SetBallSpeed), циклы орудий длиннее, урон/формулы не изменились. Выкл → всё возвращается. Missiles в полёте при смене фактора не пересчитываются (примут при следующем пуске).
...
## 16 сент. (вечер): убит главный рост памяти — кеш-CRowSet'ы теперь освобождаются (`ba071e69`)
Продолжение памяти/оптимизации. Главной утечкой был кеш: `config.BulkData.*` регенерируются **на каждый логин** и хранятся через `UpdateCache`, чей `DeepClearRep` останавливался на PyObjectEx → каждый сохранённый CRowSet навсегда тек всеми PackedRow, полями и header'ом (~МБ на логин). Ещё вчера это было нельзя чинить (строки не держали ссылку на шаровый header) — после фикса `NewRow` в PyDatabase можно:
- **`DeepClearRep` получил ветку PyObjectEx** (dbutil.CRowset/CIndexedRowset/DBRowDescriptor) со **строгим порядком: строки → keywords dict → header tuple**. Порядок критичен: dtor каждой PackedRow декрементирует СВОЮ ссылку на дескриптор, а последнюю ссылку держит keywords dict — если освободить dict раньше строк, dtor строк декрементируют освобождённый дескриптор (UAF). PackedRow для рекурсии — лист: clear() списка освобождает сами строки. Задокументировано: НЕ запускать на `PyObjectEx::Clone()` (клоны raw-шарят mList/mDict без ссылок).
- **PyDatabase**: `CIndexedRowSet::NewRow` получил тот же per-row `PyIncRef(header)`, что и CRowSet; `CFilterRowSet::NewRowset` теперь IncRef'ит общий дескриптор до того, как вложенный CRowSet его «украдёт».
- Поле-уровень temps (`SetItem/SetField` с fresh PyInt/PyFloat) продолжают течь по 1 ref — принятый baseline, системно не лечим.
- Эффект: сервер больше не растёт в памяти от логинов/маркет-кешей; frees при обновлении/инвалидации кешей (маркет, конфиг-булк, сертификаты, FW).
- Корки вчера (4 шт, ~2ГБ) из `evemu_server_cache` удалены.

## 16 сент. (день, часть 2): сервер грузил CPU — диагностика на живом + фиксы churn (`b2cc61eb`)
Симптом юзера: «сервер сильно нагружен». Замеры: server-контейнер ~98% одного ядра (игровой тред в потолке), MariaDB ~80% второго (90 МБ отдала серверу за 3 мин), хост при этом свободен (28 ядер, load 0.76). **64-битность/ядра ни при чём — игровой цикл EVEmu однопоточный by design**; разнести тики сущностей по потокам = гонки (отклонено). Реальная причина: **spawn churn** — в космосе всего ~90 ботов (базовые ~400 не набирались), а каждый `SpawnBot` = сотни последовательных SQL-запросов В игровом треде (легенда ×8 `ORDER BY RAND()`, `EnsureExtendedBotSkills` ~100 запросов, PickCorp GROUP BY, pool-pick RAND по всему пулу +3 джойна), при этом 70% спавнов бутили соседнюю систему целиком (churn load/unload — источник спама «Spawned customs NPCs» в логе). Фиксы (`b2cc61eb`):
- **Глобальный троттлинг спавна**: ≤1 бот на кластер в ~6 с (было до ~2/с суммарно).
- **Микс спавна 60% direct / 40% inbound** (было 30/70).
- **Leave-chance** в ProcessTravel 0.33% → 0.11% за тик.
- **EnsureExtendedBotSkills** на ~20% респавнов (было каждый).
- **PickCorp**: кэш кандидатов на 5 мин (был GROUP BY джойн на каждый спавн).
- **Pool-pick** без `ORDER BY RAND()` по всему пулу: кэш id-списка на 5 мин + выборка по PK.
- Инструментарий замеров: `docker stats --no-stream`, сэмпл `information_schema.processlist` (пользователь evemu видит свои коннекты; performance_schema у evemu закрыт — root-пароль рандомный), base64-скрипты через plink. Если снова «тормозит» — сначала эти два замера.
- Многопоточный тик мира и вынос SQL ботов в worker-поток — отложено: после churn-фиксов в этом нет нужды, пересмотреть при повторном насыщении.

## 16 сент. (день): точечный PyRep-аудит «Agent/Contract/Calendar/Map/Cache» — ЗАКРЫТ (`3ac81722`)
4-агентный аудит последнего непокрытого места из списка 28 авг (исследование — субагенты, фиксы вручную). Сначала верифицировали семантику PyRep.h: `SetItem`=IncRef, `AddItem`/`PyDict::SetItem`=STEAL, `PyObject` dtor владеет args, `PyPackedRow` хранит header БЕЗ ref, но DecRef его в dtor. 12 фиксов, 10 файлов:
- **КРАШ (реальный, юзер-триггер)**: `AgentBound.cpp` EpicArcStart вставлял MissionOffer с **null `bookmarks`** → первый же open journal звал `bookmarks->Clone()` на null → SEGV (5-й путь создания оффера, пропущенный фиксом `b9687ada`). Инициализирован пустым PyList.
- **UAF (латентный, ядро)**: `CachedObjectMgr::UpdateCacheFromSS` крал PyBuffer из decoder-субстрима **без IncRef** — decoder dtor освобождал буфер, CacheRecord оставался с висячим указателем. Теперь IncRef. Плюс `GiveCache(nullptr)` (CertificateMgr ×3, StationSvc при failed query) падал в MarshalStream (нет null-check в SaveStream) → null-guard в `UpdateCache` централизованно.
- **Утечка на горячем пути**: мусорные `PyIncRef(objectID)` в `HaveCached`/`InvalidateCache` (каждый cached-call; InvalidateCache 4× на изменение маркет-ордера) — удалены.
- **Мульти-владелец header**: N CRowSet'ов из одного `DBRowDescriptor` (ContractUtils::GetContractEntries) делили одну ссылку, и строки `CRowSet::NewRow()` декрементировали header без своей ссылки. Фикс дважды: центрально `NewRow()` IncRef'ит header (инвариант как в DBResultToPackedRowList), и per-rowset IncRef через `makeRowset` + creator-DecRef на выходах.
- **StationSvc::GetSolarSystem — ДАННЫЕ-баг**: ключ кеша был без systemID → первая запрошенная система отдавалась ВСЕМ клиентам (теперь ключ с ID, как в MarketMgr).
- Утечки error-paths (GetContractListForOwner, CollectMyPageInfo + unchecked GetRow), temps в 5 forwarding-овервлоадах CreateContract (PyInt/PyWString), `dropoffLocation` (clear перед DecRef — освобождает items на non-storing ветках, безвредно на storing), `choices` в StartResearch, sysID clone-and-leak в GetEntryPoint, MissionDataMgr PNG (неиниц. указатели + unconditional DecRef → wild DecRef), plasticWrap SaveItem вне null-guard, MapData кеши (re-init releases + null-fallback вместо PyIncRef(null)), MapService unchecked AsInt → IntegerValueU32.
- **Документировано (НЕ фиксить)**: `DeepClearRep` не должен рекурсировать в PyObjectEx/PyPackedRow/PySubStream (строки шарят header без ref → over-DecRef UAF); stale @todo «header never freed» в EVEDBUtils (428/461) заменены на warning — «включить закомментированный PyDecRef(header) = double-free». Системная «+1 за SetItem с fresh temp» — принятый baseline, не трогали.
- **Память — операции на живом**: мониторинг REFPTR__ERROR/REFPTR_HARD_FAIL на живом, core_pattern вернуть в apport после следующего пойманного краша. (Missile wrapper и кеш-утечки закрыты — см. секции выше; отклонённые направления — в «❌ Отклонено».)

## 16 сент. (утро): финал ночной серии — HEAD `410e7910`, ждёт пересборки юзером
- **Сборка чинилась трижды** после моих же правок: (1) скобки в `WormholeMgr::Collapse` (`5aa985eb`); (2) `Add()` с двумя перегрузками требует явный `static_cast` member-pointer (`dfc99ab3`); (3) реализация перегрузки `CreateAllianceEvent` не была добавлена в .cpp → `undefined reference` (`410e7910`). Урок: при добавлении перегрузок сервисных методов — сразу .cpp+static_cast, и проверять баланс скобок после правок в середине функции.
- **Чек-лист юзера после пересборки** (всё из ночной серии): поле на производственной палке видно; альянсовое событие создаётся; нет `#1062 itemID=0` на буте; сервер живёт через бут/бой (ракеты/дроны/варпы); коллапс ВХ без ошибок.
- **armed-диагностика**: `core_pattern=/app/server_cache/core.%e.%t` (volume → хост `/var/lib/docker/volumes/evemu_server_cache/_data/`); контейнер `restart=on-failure:10`. bt: `docker run --rm -v evemu_server_cache:/cores --entrypoint gdb evemu_server -batch -ex "bt 40" /app/bin/eve-server /cores/<core>` (gdb второй раз тот же core может не прочитать — брать свежий/копировать). После следующего пойманного краша core_pattern вернуть в apport. Ранний сегфолт (12с после старта) больше не воспроизводился — вероятно, та же баббл-дыра, закрыта `14fc5f70`.

## 15 сент. (ночь, часть 2): краш по core-дампу пойман и починен + поле POS + календарь
- **Сегфолт при старте/в бою** — bt из core-дампа (`server_cache/core.eve-server.*`, gdb в контейнере): `NPCAIMgr::Process` (NPCAI.cpp:633) разыменовывал freed-сущность из карты баббла. Корень — системная дыра: `SystemBubble::Untrack` при `m_bubble==nullptr` early-return **без стирания** из `m_entities/m_dynamicEntities`, а `BubbleManager::Remove` при null-`SysBubble()` пропускал очистку целиком → любая сущность с заранее сброшенным bubble оставалась висячей в картах. Фикс (`14fc5f70`): Untrack всегда стирает по id (промах = no-op), BubbleManager::Remove проходит все бабблы безусловно; заодно убран вечный цикл (`continue` без `++itr`).
- **Ракеты** (`b6d0ab31`): `Missile::Delete → delete this` из батча-1 откатан — wrapper освобождать нельзя, пока у ракет нет реестра владельцев (как `EntityList::m_probes`); теперь явный `m_bubble->Remove(this)` + base Delete, wrapper остаётся (утечка ~200Б/пуск — осознанно). `m_fromID`-валидация источника осталась.
- **Поле POS не было видно** (`595c9ff3`): шар рисовался из радиуса типа ForceField (≈0) → сфера нулевого размера. Теперь ball radius = `AttrShieldRadius` башни (SetRadius + фолбэк в `FieldSE::EncodeDestiny`), поле `SaveItem()` — переживает ребут.
- **Календарь** (`595c9ff3`+`dfc99ab3`): `CreateAllianceEvent` — клиент шлёт `(dateTime, None, title, description, important)`; добавлена точная перегрузка (duration=None→0, WString через GetStringContent). ⚠️ при двух перегрузках `Add()` требует явный `static_cast` на member-pointer (иначе дедукция шаблона падает).
- **Спам `#1062 itemID=0` в posStructureData/posReactorData** (`c496861e`): `ReactorSE::InitData()` переопределял базу без `StructureSE::InitData()` (луна/башня не резолвились), а `BotDeployAndAnchor`.persist был до установки itemID. Фиксы: itemID гарантируется в обоих местах, base-call добавлен, `SaveReactorData` → upsert.
- **Диагностика крашей на будущее**: `core_pattern` временно = `/app/server_cache/core.%e.%t` (видно из контейнера, лежит в volume `evemu_server_cache` → хост `/var/lib/docker/volumes/evemu_server_cache/_data/`); bt снимать так: `docker run --rm -v evemu_server_cache:/cores --entrypoint gdb evemu_server -batch -ex "bt 40" /app/bin/eve-server /cores/<core>` (gdb повторно тот же core может не прочитать — копируй свежий). Контейнеру включён `restart=on-failure:10`. После поимки следующего краша core_pattern вернуть в apport.

## 15 сент. (ночь): аудит памяти — батчи 1+2 закрыты
Полный 4-агентный аудит PyRep-владения/UAF/утечек. **Контракт вызовов (ВАЖНО знать)**: `SendSingleDestinyUpdate/SendSingleDestinyEvent` теперь **поглощают** тапл (`*up=nullptr` + DecRef на всех путях, включая early-return) — новые вызовы НЕ должны DecRef'ить после; `Multicast/Broadcast(*payload)` обнуляют указатель вызывающего; `SendNotification` поглощает 1 ref (не обнуляет указатель).
- **Батч 1 (`896cbd7b`+`46a79993`)**: `_SendException` (`*payload=nullptr` — double-release на каждом UserError, класс краша «IncRef on deleted»); 4× Multicast/Broadcast «consumed»; 3× CorpRegistryBound UAF (IncRef ДО SendNotification); DroneSE ScoopAndDelete/pendingRemoval — ClearFromTargets+detach перед `delete this`; ClearDronesAssignedTo чистит и `m_pShipSE` (сеттер `SetHomeShip`); ItemFactory `end()`-чеки в GetItemContainerRef/Inventory; ActiveModule::ConsumeCharge — зомби-заряд (кончился → Deactivate+null, было «бесконечный боезапас»); Missile — источник валидируется по `m_fromID` через GetSE (стрелок мог умереть в полёте), `Delete()` освобождает wrapper (раньше тек SE на каждый пуск); 4× SafeDelete ботам в BotMgr; `~Client` — чистка m_pendingUpdates + null-check станции.
- **Батч 2 (`9ad6885e`)**: пробы переживали Client → `EntityList::RemoveClientProbes(charID)` из `~Client` ДО `SafeDelete(m_scan)`; `DestroyProbe` стирает из реестра m_probes (иначе тикал freed); MissionDataMgr — часовая чистка m_xoffers + `DELETE FROM agtOffers WHERE NOT (dateCompleted=0 AND stateID<3) AND expiryTime < cutoff` (30 сут); EncounterServer — завершённые офферы/мининг-сайты стираются; `InventoryItem::Relocate` — RemoveItem из старого контейнера (заряд числился в слоте И трюме); `AbandonDrones` чистит m_drones; CivilianMgr — double-delete item убран, TransferCrossSystem больше не удаляет item (RemoveNPCFromList+RemoveEntity); SpawnMgr m_capitalWaves erase; AutoPay-мап sweep; инкурсия ClearDamageData безусловно.
- **Известное НЕисправленное — ЗАКРЫТО (`b9687ada`)**: `MissionOffer::bookmarks` теперь `RefPtr<PyList>` (структуры копируются по значению — refcount решает double-free; 4 места присваивания → `RefPtr<PyList>(new PyList())`); WormholeMgr::Collapse освобождает оба SE-wrapper'а (~WormholeSE тривиален); CachedObjectMgr — `DeepClearRep` рекурсивно освобождает дерево репы (PyTuple/PyList/PyDict → clear()+дети). Контракт: PyRep base НЕ имеет clear() — только контейнеры.
- **DB-контракт (`02085b03`)**: `DBcore::DoQuery_locked` рвёт соединение ТОЛЬКО на коннекшен-ошибках (2006/2013/2003/2055); обычные SQL-ошибки (#1054/#1062/...) больше не вызывают реконнект на каждом следующем запросе.

## 15 сент. (ночь): фикс пропавшего фита челоботов + киллы бот-убийц на киллборде
- **Фит пропал из-за reuse-ветки, НЕ из-за груза** (`8099c00c`): с `a340c757` («reuse пробуется ВСЕГДА») переиспользуемые пилоты пула спавнились БЕЗ `useShipType/useFit` — легенда (`botKillmailLegends`) читалась только при минте НОВОГО пилота, а в reuse-ветке и в retry-ветке «уже в системе» поля пустые/чистились → фит-блок `hullType == useShipType && !useFit.empty()` не запускался никогда, при этом `MaterializeShipLoad` (груз) безусловный → корабли с грузом и нулём модулей (проверка БД: 140 кораблей, 0 модулей, 66 предметов трюма). **Фикс**: для pooled-пилота его собственная легенда (корабль+фит) восстанавливается по имени из `botKillmailLegends` (`ORDER BY killmail_id DESC LIMIT 1`) в ОБЕИХ ветках; все 2500 пилотов пула имеют легенду (проверено JOIN'ом). Мирные профессии по-прежнему на проф-халлах без боевого фита (by design), Hunter/RatHunter получают полный киллборд-фит.
- **Киллы бот-убийц не атрибутировались** (там же): при убийце-челоботе в `finalCharacterID` писался **corpID** бота (PlayerBot — NPC SE: нет пилота/дрона) → киллборда (join `chrCharacters` по `finalCharacterID`) не видела ни одного килла от бота (83 потери ботов записаны, 0 киллов). Фикс в ОБЕИХ писателях киллмайлов (`PlayerBot::RecordBotKillMail` и `Ship::Killed` в `Damage.cpp`): резолв `PlayerBot` → `GetBotCharID()`. Заодно владелец врека/лута — бот-пилот, а не корп.

## 15 сент. (вечер): always-on системы ботов + контракты трейдеров
**Модель населения изменена**: раньше `BotMgr::Process` реапил ботов при `PlayerCount()<1`, а система выгружалась через ~60с без активности → цепочки замирали без игрока.
- **Always-on системы** (`9bb7402b`): `EnsureAlwaysOnSystems()` (BotMgr) один раз грузит набор — **все trade-хабы** (`botTradeHubs`) + список `AlwaysOnSystems` из конфига (comma-separated systemID) — и помечает их `SystemManager::SetPersistent(true)`. Персистентная система НЕ выгружается (`SystemActivity()` возвращает true) и держит базовое население 24/7.
- **База vs игрок**: новый конфиг `BaselinePerSystem` (напр. 30) — боты живут в always-on системе всегда; `MaxPerSystem` (напр. 60) — топ-ап при онлайне игрока. `PopulateSystem` считает cap по наличию игрока; ре-ролл target, если cap сменился.
- **Реап**: `BotMgr::Process` реапит только НЕ-персистентные системы без игрока. `ProcessDocking` теперь докирует и в персистентных системах (докнутая экономика/трейдеры работают без игрока).
- **Симметричный список** (в `utils/config/eve-server.xml` и в живом `/opt/evemu/config/eve-server.xml`): Jita 30000142, Motsu 30002819, Nonni 30001401, Hek 30002053, Rens 30002510, Pator 30002544, Amarr 30002187, Tash-Murkon Prime 30001671, Dodixie 30002659, Oursulaert 30004969, Villore 30004993, Yulai 30005204 (4 империи + центр, траффик во все стороны). Хаб Jita добавляется автоматически из `botTradeHubs`.
- **Контракты трейдеров** (`0517ea31`): `PlaceBotItemContractAt` — докнутый трейдер выставляет реальный склад как публичный **item exchange (тип 1)** или **аукцион (тип 2)** на своей станции (предметы лочатся в контракт owner→1 + `ctrItems`); вызов ~4% экономических тиков трейдера. `AnnounceBotContract` — линк контракта в **локал системы контракта**, только если там есть реальный игрок, throttle 1 линк/система/~3 мин. Формат ссылки клиента (из декомпила `contracts_py`): `<a href="contract:<systemID>//<contractID>">Title</a>`. Курьерские контракты (и виртуальный, и реальный склад) тоже анонсируются; `PlaceBotCourierContractAt` теперь возвращает contractID.
- **Сборка**: `uint64` в этой кодовой базе НЕ существует (закомментирован в `eve-compat.h`) — использовать `int64`; `unix` — предопределённый макрос GCC (нельзя как имя переменной). Фиксы: `b70213ac`, `9bb7402b`.

## 15 сент.: Telegram — дайджест киллов, онлайн/оффлайн, вечерний админ-отчёт, бот-POS схемы
**Миграция `20260915000000-telegram_state.sql`** (ОБЯЗАТЕЛЬНО применить — добавляет в `srvStatus` колонки `lastOffline`, `lastDigest`, `lastAdminReport`, `bootCount`, `crashCount`, `lastReportBoot/Crash/Commit`, `cleanShutdown`). Без неё телеметрия/персистентность не работает (в лог DATABASE__ERROR), но сервер не падает.
- **Дайджест киллов — раз в сутки, переживает рестарт**: маркер `srvStatus.lastDigest` вместо статики (раньше `sLastKillDigest=0` на каждом боте → дайджест слался сразу после каждого ребута). `DailyKillDigestTick` (BotMgr.cpp) читает БД раз в бут/сутки (`sNextDigestCheck`), шлёт если `now - lastDigest >= 86400`. Формат переделан: **1 строка на килл**, `N) время · жертва <корп> — корабль [класс] · система (регион) · ⚔ убийца (корабль) · dmg`; время считается в C++ (`FormatKillTime`: FILETIME → localtime → `%d.%m %H:%M`) — SQL `DATE_FORMAT` на живом сервере давал мусор (`1159753550.No child processes`).
- **Онлайн/оффлайн — дебаунс (юзер выбрал)**: `eve-server.cpp` — при старте `online` шлётся только если простой ≥ 10 мин (`GetLastOffline`); при стопе `offline` шлётся только если сессия была ≥ 10 мин (`s_serverStartTime`), а `lastOffline` пишется всегда. Быстрый ребут/пересборка → тишина. `AnnounceOfflineIfReal()` (forward-declared), вызывается в обоих shutdown-путях.
- **Вечерний админ-отчёт** (BotMgr.cpp `DailyAdminReportTick`, 21:00 по серверу, только админ-канал): 🔄 перезагрузок, 💥 падений, 🧩 коммитов (дельта с прошлого отчёта), 📦 билд. Падения считаются через `cleanShutdown`: `RecordBoot()` на старте — если прошлая сессия была dirty (cleanShutdown=0), `crashCount++`; `RecordCleanShutdown()` на чистом стопе. Коммиты — из `/app/etc/build_info.txt`, который пишет `tools/write_build_info.sh` на хосте после `git pull` (ключи `commit`/`count`/`built`); дельта = `count - lastReportCommit`.
- **Бот-POS схемы** (см. секцию «POS-схемы Crucible» ниже) + фикс запроса крафта в POS.

## 11 сент. (поздний вечер): залипший скрамбл, буквы, топливо бот-POS
- **«Warp drive is disrupted» залипал после убийства Sansha-скрамблера** (`13732d87`): регрессия утреннего фикса краша дронов (`d53a632c`). В `NPC::Killed` блок `m_targMgr->Destroyed()/ClearFromTargets()` стоит ПЕРЕД EWAR-очисткой, а она читает `TargetMgr()->GetTargets()` — `Destroyed()` уже опустошает список → скрамбл на корабле игрока снять было некому. Фикс: EWAR-очистка (скрамбл+вэб+paint+стоп-бимы) перенесена ВЫШЕ разрушения таргет-листа. Порядок критичен — не переставлять.
- **Диагностика варпа** (`19e0f94f`): при отказе варпа (DestinyManager WARP-ветка) теперь логируется сила `WarpScrambleStatus` (1-2=NPC-скрамблер, 100=осадка/триаж, тысячи=MWD-баббл) под `DESTINY__WARP_TRACE`.
- **Сборка**: `c751d7d1` — восстановлена строка `PyList* dirList = new PyList();` в `ItemSystemEntity::MakeSlimItem` (выпала при правке dunDirection).
- **Буквы (bookmark) — формат**: сервер для координатной закладки пишет `itemID = solarSystemID`, `typeID = 5 (invTypes::SolarSystem)` (`BookmarkService.cpp:141`), НЕ `itemID=0/typeID=0` (клиент трактует 0 как «закладку на объект» → ведёт на станцию). `x/y/z` — метры. Проверка: `SELECT ... FROM bookmarks`.
- **Буква Mr Tort №68** (Laic 30003049): сначала был неверный формат (вёл на станцию) → исправлен; затем координата была `tower.x + 50 000 000` (50 000 км!) вместо `+50 000` (50 км) → в космосе пусто. Итог: `x = 83166874642` (= башня `83166824642` + 50 км), `y/z` башни, `itemID=30003049`, `typeID=5`. Список закладок клиент кэширует — после правки нужен перелог.
- **Топливо бот-POS** (`ed922ec7`): `TowerSE::BotEnsureFuel(hours=720)` — заливает в трюм башни топливо (`invControlTowerResources` → Fuel Blocks по типу) до 30 суток и, если башня уже reinforced/offline, `SetOnline()` (возврат в Online → видима). Вызывается из `OnBotAnchorComplete` (деплой) и на первом тике `Process()` для башен с `customInfo='botpos'` (перезагрузка/бут → дозаправка+подъём). `DeployBotPOS` ставит `customInfo='botpos'` и при наличии башни у корпа дозаправляет её (не плодит дубль). Заодно починен запрос-дубликат: в `entity` НЕТ колонки `groupID` — джойн `invTypes` (старый запрос падал и никогда не срабатывал).
- **Уже стоящие 3 башни** (Laic: `140838399` Gallente CT corp 1000171, `140839016` Sansha CT Medium 1000171; Avele: `140832953` Angel CT Medium 1000168) помечены в БД `customInfo='botpos'` → после пересборки+бута заправятся и поднимутся в Online. Были `flag=145 (StructureInactive)` = reinforced (топлива не было).
- **Раскладка бот-POS + порядок онлайна** (`76b954c5`): модули больше не стоят линией по x. `DeployBotPOS`: производство (Assembly Array + Silo) кучно — 1.8 км друг от друга, ~2.2 км от башни (≤2500 м, как просил юзер); 2 орудийные батареи сверху/снизу (`±0.60·R` по Y), web-батарея на кольце (`+0.70·R` по X); `R` = радиус силового поля башни (`AttrShieldRadius`, дефолт 20 км) — всё внутри поля. Порядок: **башня (якорь→топливо→онлайн), затем каждый модуль (якорь→загрузка→онлайн)**. `TowerSE::BotOnlineModules()` сканирует систему на POS-модули того же владельца в радиусе поля и онлайнит их после подъёма башни (иначе после перезагрузки модули оставались оффлайн). Существующие 3 POS переставлены в БД (SQL, не в репо).
- **Производственники (проверка БД)**: POS/офисы/PI/закупки работают (см. ниже), но готовой продукции пока нет — крафт-этап редок. `botColonies` = 3 активные колонии.
- **Проверить после пересборки** (следующая сессия): (1) башни Laic/Avele в Online + топливо в трюме; (2) модули вокруг башни кучно/по периметру (не линией), онлайн; (3) закладка №68 Mr Tort ведёт в космос в 50 км от башни; (4) новый деплой POS (докнутый производственник в хайсеке) — раскладка и порядок «башня→модули». Миграций к этой серии нет (правки БД — вручную, не в репо).

## 📐 POS-схемы Crucible (референс, записал юзер 15 сент.) — для бот-POS
Классические доктрины расстановки ПОС (историческая механика Crucible). Бот-POS теперь выбирает одну из схем детерминированно по `corpID % N` (см. `BotMgr::DeployBotPOS`), чтобы вселенная выглядела разнообразно, а не одним униформ-фитом.
- **Deathstar («Звезда Смерти»)** — максимум урона: много орудийных батарей (Large Artillery/Lasers — дальнобойность/альфа/без боеприпасов у лазеров), по 6 «компасным» точкам (верх/низ/бока) без слепых зон; крупные сверху/снизу, средние/малые кольцом. Плюс: усилители щита (hardeners), scram/web (удержание цели), нейтрализаторы (осушают логистов), немного ECM (мешать логистике).
- **Super-Hardened («Сверхзакалённый»)** — максимум выживания: множество усилителей щита (сопротивления до ~68/68/68/71%), десятки батарей сенсорного подавления (Sensor Dampeners — не дать стрелять с дальней дистанции, заставить подойти ближе), минимум орудий. Слабость: 10+ линкоров с командником продавливают; без ECM логистику врага не сорвать.
- **Промышленный/утилитарный** — оборона вторична:
  - *Добыча луны*: Moon Harvesting Arrays + Silos (оборона минимальна, тыл).
  - *Производство*: Labs + Assembly Arrays, компактно.
  - *Червоточины*: Ship Maintenance Array (смена кораблей) + Corporate Hangar; часто ДВА хангара (дешёвые корабли — всем, дорогие — доверенным).
- **Реакторный конвейер (T2, Large Caldari Tower — больше всего CPU)**: Экстрактор → Coupling Array (буфер) → Силос(вход) → Простой реактор (Simple BP) → Силос-буфер → Сложный реактор (Complex BP) → Выходной силос. Простой реактор даёт 200/ч, сложный жрёт 100/ч → буферный силос обязателен. Выгрузка — через Offline силоса (для этого Coupling Array, чтобы не стопить добычу). Лимит CPU: 2 экстрактора + 4 силоса + 2 реактора на Large CT = 0 CPU.
  ```
  [Луна] → [Экстрактор A] → [Coupling] → [Силос вход1]
         → [Экстрактор B] → [Coupling] → [Силос вход2]
        → [Простой реактор] → [Силос-буфер] → [Сложный реактор] → [Выходной силос]
  ```
- **Сборка кораблей/модулей**: Assembly Array (Small/Medium Ship, Drone, Ammunition…) + Labs (Copying/ME/PE) + Hangar Array. Для КАПИТАЛОВ обязательна Крупная башня + Capital Ship Assembly Array.
- **Практика**: все модули — внутри силового поля; ручное управление пушками — пилот в **15 км от башни**; Cynosural System Jammer окружают scram/web (не дать врагу встать в RR-фит); планировали через POS Planner (браузер). Лимиты CPU/Powergrid контрольной башни — главное ограничение.
- **DB-факты (проверено 15 сент.)**: `entity` НЕ имеет колонки `groupID` (join `invTypes`); группы модулей — Control_Tower 365, Assembly_Array 397, Silo 404, Ship_Maintenance_Array 363, Mobile_Laboratory 413, Moon_Mining 416 (=Moon Harvesting Array 16221), Mobile_Reactor 438 (Simple Reactor Array 20175 / Complex Reactor Array 16869), Electronic_Warfare_Battery 439, Sensor_Dampening_Battery 440, Stasis_Webification_Battery 441, Warp_Scrambling_Battery 443, Shield_Hardening_Array 444, Corporate_Hangar_Array 471, Energy_Neutralizing_Battery 837. Target_Painting_Battery (877) — 0 published, не использовать. Simple_Reaction (436) — это reaction-формулы, НЕ модуль.
- **Реализовано (`a276e589`)**: `DeployBotPOS` выбирает схему по `corpID % 5` (0 Deathstar, 1 Super-Hardened, 2 Moon-reaction, 3 Shipyard, 4 Wormhole); `spawnModule` теперь выбирает SE-класс по groupID как `DynamicEntityFactory` (ArraySE/ReactorSE/BatterySE/WeaponSE/StructureSE); стоимость считается из реальных `basePrice` заякоренных модулей. Производственный кластер (array+silo, ≤2500 м) присутствует во ВСЕХ схемах — производитель крафтит у башни.
- **Башня/поле/резисты — рабочие (`fe7b26c4`)**: (1) `BotDeployAndAnchor` сохранял состояние INSERT'ом (`SaveBaseData`) в существующую строку → падал по дублю PK → все бот-башни после рестарта грузились **Unanchored** (в БД все 6 имели state=0); теперь `UpdateBaseData`. (2) `BotEnsureFuel` самолечит Unanchored-башню (якорь у луны, линк луны/бабла), ставит пароль (поле создаётся ТОЛЬКО при непустом пароле!) и поднимает Online + поле. (3) `BotOnlineModules` больше не онлайнит модули у мёртвой башни. (4) `SetOnline` персистит harmonic+password (новый `PosMgrDB::UpdateTowerData`). (5) **Поле работает**: `SystemBubble::IsInProtectedField` — корабли (и дроны/NPC/боты) внутри онлайн-башни в радиусе `AttrShieldRadius` не таргетятся (оба варианта StartTargeting) и не получают урон (бэкстоп в `SystemEntity::ApplyDamage`); башни/модули — атакуемы, как в EVE. (6) **Харденеры работают**: `TowerSE::ApplyHardeners()` — онлайн Shield Hardening Arrays (группа 444) внутри поля снижают резонансы щита башни (271-274) по бонусам 1489-1492 (мультипликативный стак, кэп 90%, идемпотентно); вызов из Init/SetOnline/BotOnlineModules.
- **Замкнутая лунная цепочка (`fe7b26c4`)**: `ProcessMoonPOSProduction` (вызов из `ProcessDockedIndustrialEconomy`) — харвестеры (416) копят сырые Moon Materials (427) в ангар владельца; простой реактор: 2×100 сырья → 100 Intermediates (428); сложный (≥2 реакторов): 200 пром. + 100 сырья → 100 Composite (429, T2-сырьё). В дампе Crucible НЕТ формул реакций (`ramTypeRequirements` пуст для группы 436) — упрощённые 2→1 конверсии. Выгрузка через существующие курьерки/продажу, как с рудой.
- **Охрана/боевые модули**: WeaponSE (сентри 417/426/430/449) стреляют через POS_AI (заряжены chargeGroup1), BatterySE web/scram/neut — реализовано ранее; Deathstar-схема даёт 6 орудий + scram/web/neut/ECM. Резисты — см. харденеры выше.
- **Заодно починен запрос крафта в POS** (`BotMgr.cpp` `ProcessDockedIndustrialEconomy`): `SELECT ... FROM entity WHERE ... groupID=%u` использовал несуществующую колонку → запрос всегда падал → `craftAtPOS` всегда false (крафт шёл на станции). Теперь join `invTypes` по `typeID`.

## 11 сент. (вечер): полный перенос фиксов инкурсий на обычные/слиперские аномалии + волновая модель
Юзер: «проверь все аномалии, включая обычные и слиперов, и исправь по ним то, что мы пофиксили по аномалиям вторжения» (сентри, ворота, ориентация, positions,
разнесение структур/газа, NPC неверной фракции). Коммит сериал `8e4af418` + `892093b1`.
- **Ориентация/недолёт ворот** («всегда стоят на 180°»): клиент рендерит ворота через `spaceObject.WarpGate` → `SetStaticDirection() → AlignToDirection(dunDirection)`; **`dunRotation` для ворот НЕ читается** (это поле только для структур/таможни → `SetStaticRotation`), поэтому правки 0/180 не влияли. `AlignToDirection` кладёт локальную +Z модели на `-dunDirection` → dunDirection отправляем с инвертным знаком. Правка центральная (все ворота: инкурсии/обычные/слиперы).
- **Врата за последней структурой**: `MakeDungeon` (обычные/слиперы) теперь ставит ворота за max +x extent комнаты +9 км (фloor 30 км) вместо фиксированных +30 км — то же правило, что у волновых ворот инкурсий.
- **Сентри-башни снова летали**: корень — `Target()`/`Targeted()` звали `SetChasing()` напрямую, минуя stationary-гвард `CheckDistance()`. Фикс: гварды в `SetChasing/SetFollowing/SetEngaged` + для стационарных `Target()/Targeted()` только лок+стрельба с места.
- **Разнесение структур/газа по объёму** — уже было общее (`SpawnDecorations`, margin ∝ радиус), используется всеми аномалиями — догонять нечего.
- **NPC неверной фракции**: `DoSpawnForAnomaly` получил `forceFactionID`; `MakeDungeon` передает `dData.factionID` — чужие пайраты (Serpentis-корпуса на Angel-сайте) спавнятся с фракцией сайта, статы/тир сохраняются.
- **Волновая модель (юзер выбрал «полная»)**: миграция `20260911000000-dungeon_waves.sql` — колонки `wave`/`isTrigger` в `dunRoomObjects`; боевые сайты Спящих (rooms 4001-4099): wave1 = Patrollers/Defenders, wave2/3 = Sentinels, последний Sentinel = trigger. `DungeonMgr::WaveRuntime` (по bubbleID кармана): wave>1 стейджится, спавн по зачистке или смерти trigger-NPC (`OnDungeonNPCDestroyed`, хук в `SpawnKilled` ДО легаси-чейнинга случайных ратов). Инкурсии (дуng 2100-2133) не задеты — своя цепочка.
- **Кап-эскалация Спящих** переехала в `SpawnMgr::TryCapitalEscalation` (site-level): волна 6/8/6/8 (=до 28 стражей) на КАЖДЫЙ НОВЫЙ кап (кроме Rorqual/группы 883);guards tier = tier сайта (Sleepless 30195 / Awakened 30205 / Emergent 30214), level 1/3/5 для scale. Раньше это крутилось у КАЖДОГО слипера (10 слиперов = 10 волн) и максимум было 2.
- **Памятка**: юзер уже подтвердил ворота/вэйп-прилёт InterBus таможню; эта серия — след. пересборка. Миграцию `20260911000000` НЕ ЗАБЫТЬ применить (иначе `dunRoomObjects.wave` колонки нет → GetAllDungeonData упадёт).

## ✅ Статус 11 сент. (подтверждено юзером)
- **Сервер пересобран на актуальную версию** (последние коммиты, включая варп-лендинг `9fd37fe9`/`d1a9dfb3`/`b186900a` и дедуп customs NPC `74d66d84`).
- **Варп — ОК**: дистанция прилёта корректная, телепортов в конце варпа НЕ обнаружено (капитал, 10–35 AU). Недолёт 4.4–5 км и end-of-warp телепорт закрыты.
- **Customs offices InterBus** — наблюдаются в игре (офисы на хайсек-планетах).
- **Боты (POS/PI/инкурсии/профы)** — отложено, смотрим позже.
- **Портал** — актуальный, проверен, всё работает. **Web Push пока НЕ включён (нет TLS-сертификата)**; включать позже на HTTPS (VAPID + cron). Роадмап (ISK-fallback, проверки админ-выдачи) — позже.
- При следующей пересборке не забыть вернуть live `log.ini` в ERROR-only (см. TODO ниже).
...
## ✅ WARP — РЕШЕНО (11 сент.): дистанция прилёта корректная, телепортов в конце варпа нет. Ниже — история (10 сент.) для контекста.
### 🔴 WARP — МИКРОРЫВОК В КОНЦЕ (10 сент., не решено, отложено) — закрыто 11 сент.
...
- **Быстрый тест инкурсии**: GM-команда `/incursion` (`33ae4809`, роль GMH) — стартует инкурсию в созвездии и сразу спавнит сайт в текущей системе (без ожидания/шанса).
- **Белые NPC инкурсии** (`eb1cf9c5`): заглушки Sansha групп 1051-1056 (типы 2190-2192, без дамаг-атрибутов) теперь заменяются на реальных Sansha (группы 565/566/567) и в **первом кармане** (`MakeDungeon`), а не только в волновых.
- **Ворота ускорения** (`1f2df260`/`40358eb2`/`993978c4`/`12ddef0c`): убран лишний `+180°` в `dunRotation` (смотрели навстречу прыжку); позиция ~30-34 км за аномалией (5-9 км за последней структурой). Волновая цепочка устойчива к смене бабла (фолбэк на единственный сайт в системе) + лог `Incursion wave N gate placed`. ⚠️ на момент записи ворота после зачистки волны ещё НЕ подтверждены — проверить после пересборки `12ddef0c` по логу.
- **Краш при убийстве Sansha дронами** (`d53a632c`): `NPC::Killed` теперь `m_targMgr->Destroyed()+ClearFromTargets()` ДО волновой очистки — дрон держал указатель на убитого NPC → UAF SIGSEGV.
- **Залипший скрамбл** (`c7905979`): дроны отслеживают `m_ewarTargetID` и снимают warp scramble/web/paint в `SetIdle` (цель могла выпасть из TargetMgr). Рантайм-атрибут, в БД не пишется → снимается рестартом.
- **Декорации** (`61624881`): крупные структуры и облака разносятся с общим учётом дистанции (margin растёт с размером, 16 попыток).
- **Боты следуют за игроком** (`33ae4809`): `ReapBots()` вызывался впервые — теперь в системах без реальных игроков боты реапится (троттл 30с), иначе они вечно оставались в старой системе.
- **Ростер**: ребаланс учитывает профессию 8 (Industrialist) и при пустой профессии делает сид (до ~1/8 самой массовой) — производственники реально появились в пуле (было 76).
- **Кап ботов**: reuse пробуется ВСЕГДА (был гейт на COUNT, который под нагрузкой возвращал 0 → спавн новых сверх капа); COUNT — только для пустого пула; кап перепроверяется на ретраях топ-апа (`a340c757`). `TrimPilotPool()` на старте режет до капа.
- **Диск**: хост упирался в 100% (db не стартовал) — вычищено ~189 ГБ (старый `backup_evemu_20260826.sql` 4.8G + `docker container prune` 23.86G + `docker image prune -af` 165.6G). Периодически чистить.
- **Профессия Industrialist** (ролл ~7%, industrial hull): рекурсивная производственная цепочка по `invTypeMaterials` (`BotBuyStock` локально → `BotBuyStockRemote` импорт из региона), POS у луны (Control Tower + Assembly Array + Silo + орудийные батареи + web), крафт физически в Assembly Array → перевоз на станцию, PI-цепочки P1→P4 по `schematicsTypeMap` (колония `botColonies`), свой Customs Office на планете колонии, логистика курьеркой/продажей.
- **POS-оборона**: `POS_AI` обобщён на `StructureSE` (BatterySE web/scram/neut + WeaponSE урон, расход зарядов chargeGroup1); наведение учитывает стендинг/security-status/войну башни + хайсек-гейт; **ручное управление** (AssumeStructureControl: Starbase Defense Management + 15 км + обездвиживание; `dogmaIM.AddTargetOBO/RemoveTargetOBO` ручная цель, POS_AI приоритизирует); **боты-охранники** (2-3 Hunter корпа, орбитят башню, ассистят ручную цель; появление login-at-station→варп или login-at-POS).
- **Кастом-офисы**: запуск орбиталки биндится к ближайшей планете (`customInfo=planetID`), иначе анкор невозможен; миграция сидит NPC InterBus-офисы (4318, corp 1000148) на всех хайсек-планетах (~9339).
- Правки в портале (evemu-portal): разнесение конфига (`config.php` user / `const.php` engine / `config.sample.php`), `/mail` однострочные строки + аватары уведомлений + полный маппинг Notify::Types, SMTP/2FA/правила, фикс навбар-дропдаунов (тап/клик) — см. AGENTS evemu-portal.

## ПРАВИЛА ЮЗЕРА (записано 9 сент. — соблюдать!)
- **Кап ботов**: максимум пилотов = `MaxTotalPilots` из конфига, жёстко. Достигнут → генерация НОВЫХ символов останавливается; спавн ПЕРЕБИРАЕТ существующих ботов с их текущими профессиями (`botMemory.profession` сохраняется между респавнами). Пул растолстел (сегодня было 3907 при капе 2500) — `BotMgr::TrimPilotPool()` на старте удаляет САМЫХ НОВЫХ сверх капа (и их портреты/мейлы/киллы/скиллы/итиемы) и пул самолечится.
- **Портреты**: если ESI (images.evetech.net недоступен из RU) не скачался → curl через `--proxy` (общий с <telegram>) → если и это не вышло, **рандомная процедурная генерация зависит: `eve-server genportrait <path.png> <seed>`** (в BotMgr.cpp: детерминированный PNG 512×512 «пилот bust», zlib; тот же seed → то же лицо) прямо в imageDir/Character/ — пилот без лица не остается.
- **Тренинг**: бот качается по своему профессиональному скилл-пулу (LIKE-патерны в ProcessBotTrainingBatch). Ребаланс ростера — ТОЛЬКО при нехватке какой-то профессии (<1/4 от равномерной доли), раз в 30 мин: переезжают **низкоскилловые** пилоты самой многочисленной профессии на недостающую; **скиллы остаются на чаре** (прогресс не теряется), меняется только botMemory.profession → халл обновится на следующем респавне.
- **Био**: bio МОЖЕТ меняться. Пустое био → всегда генерируется новое; существующее обновляется редко (12% на респавн). Пулы био включают: псевдографику (ASCII-арт), EVE-мемы и настоящие ссылки на картинки evetech. ⚠️ Фикс маппинга: пул бил по enum 1..7, а передаётся 0..7 — был сдвиг (майнер получал био раттера и т.д.).
- **Чат (DeepSeek)**: русский язык по восприятию РАЗРЕШЁН (RU-сленг в промпте). Ответы grounding по состоянию космоса: имя системы + sec (кешеванный lookup), inCombat/playerNear, и ТЕКУЩАЯ работа (BuildBotSmalltalkLine) — бот знает где он, что делает и кто рядом.
...
- **Единая модель петиций** (портал ↔ игровой F12): серверный `ProcessPetitions` в `APIAdminManager` переписан на общие `portal_petitions`/`portal_petition_messages`/`portal_petition_categories` — `PetitionList`(админ, все+категория), `PetitionMine`(только свой accountID, deleted=0), `PetitionCreate`(категория+senderid; petition.characterID=0 → видна всем чарам аккаунта в игре, сообщение несёт реальный senderID), `PetitionMessages`(тред; владелец по accountid), `PetitionAddMessage`/`PetitionCancel`(игрок, ownership+open check), `PetitionReply`(админ, isGM=1 в тред), `PetitionClose`, `PetitionCategories`(дерево, язык whitelist). Ответ админа — сообщение в тред (не CONCAT в body). Портал `/petitions` (создать с группой→категорией, свои, тред, ответ/отмена) и `/admin/petitions` (все, тред, ответ GM, закрыть) — переписаны. F12-петиции подтверждены юзером («заработали»).
- **API-краш NULL**: `std::string(row.GetText(N))` на NULL-колонке бросал `basic_string: construction from null` и гасил соединение APIServer (пустой ответ). Фикс: хелпер `SafeStr()` + все текстовые/датовые поля петиций/timecodes/news выводятся через него. (Remote-коммит `b236dfb3` чинил touchDate отдельно; наш `d74882d0` — системно, после rebase суперсет сохранён.)
- **POST-тело резалось на входе** (`d8325063`): `read_until("\r\n\r\n")` в `APIServer.cpp` читал только до конца заголовков; длинное тело (новость с портала), пришедшее отдельным TCP-сегментом, терялось → в `serverNews.body` сохранялось 170 байт с обрывом на «изуче» и в ТГ уходило то же. Фикс: после заголовков дочитываем `Content-Length` байт через `boost::asio::read(transfer_exactly)`.
- **Архив новостей** (`b994eeac`+портал `955be91`): `NewsList.xml.aspx`/`NewsResend.xml.aspx`(повторная отправка в ТГ — читает тело из БД)/`NewsDelete.xml.aspx`(полное удаление, только числовой id). Портал `/admin/news` — таблица архива с кнопками «В ТГ» и «Удалить». SQL-безопасно (DoEscapeString на INSERT, числовые ID, xmlEscape на вывод).
- **Расширенный дайджест киллов** (`d9a17f9d`): вместо «• Name (Ship) — Sys · damage N» каждый топ-килл теперь: `🕐 время · жертва <корпа> — шип [класс]`, `📍 система (регион)`, `⚔ убийца/шип [класс] · dmg` (HumanizeIsk: 30.0k/1.2m/3.4b). SQL — один запрос с джойнами chrCharacters/crpCorporation/invTypes/invGroups/mapSolarSystems/mapRegions. Логика вынесена в `BotMgr::BuildKillDigestText(limit, sinceSql)`/`HumanizeIsk` (объявлены в BotMgr.h) для переиспользования из `/topkills` и `/last`.
- **Команды Telegram-бота** (`ff8d2875`): новый `npc/TelegramCmd.{h,cpp}` (в CMake) — фоновый поток long-polling `getUpdates` (curl через fork/exec+pipe, мини-JSON-сканер update_id/chat.id/text без внешней JSON-либы; offset акается на каждый апдейт даже если не команда/чужой чат). Один поллер на уникальный токен (dedup; роль = по chatID). Ответы — `sendMessage` с `--data-urlencode text@file` (текст юзера не в shell). sDatabase потокобезопасна (Mutex в RunQuery), `/online` читает атомики sEntityList/sBotMgr как API-поток. Запуск `TelegramCmd::Start()` в eve-server.cpp после БД, `Stop()` в обоих shutdown-путях.
  - **Игроки**: `/help /online /topkills /market /who <имя> /last`
  - **Админы**: `/help /status /flags /petitions /accounts /bans <ip>`
- **Билд-фиксы**: `8b88b658` (NewsResendXML: `"📢 "+const char*` → SafeStr). Коммиты сессии в origin/master: `d74882d0, d8325063, b994eeac, 8b88b658, d9a17f9d, ff8d2875` (после rebase на ~28 remote-коммитов: telegram, security/RMT-аудит, банд-причины, скилл-тренинг ботов).
...
**Что найдено:**
- `SpawnBot` (BotMgr.cpp:296,342) читает `useFit` (JSON `fitted_item_ids` из `botKillmailLegends`), но **НИГДЕ не парсит/не применяет** — мёртвая переменная. Вместо фита бот получает синтетику: атрибуты (AttrGfxTurretID по расе, damage и т.д., BotMgr.cpp:700-766), модули-предметы в корабль НЕ спавнятся.
- EDK sotzone отдаёт фит по `?a=eft_fitting&kll_id=N` (EFT-текст, модули по слотам через пустые строки). Имена модулей резолвятся в typeID нашей БД (Rocket Launcher II=10631, Small Shield Extender II=380, 1MN MWD=440, Ballistic Control II=22291, Hawk=11379...). Инвалюабельность: `?a=kill_detail&kll_id=N` (жертва/corp/ship), листинг `?a=kills&y=Y&m=M&page=N` (30/стр, страницы по `page=`).
- В `botKillmailLegends` уже было 225 sotzone-записей (killmail 64988-65494, свежие) с фитами — прошлый пробный импорт; уникальный ключ (killmail_id, character_id).
**Сделано:**
- `tools/import_edk_fits.py` (`79494c7a`+`f3c15934`): скрейп полного sotzone 2007-2011, resume-таблица `edkProcessedKills`, INSERT..ON DUPLICATE KEY UPDATE, лог прогресса. Запущен фоном на сервере (`nohup ... /tmp/edk_full.log`, `/opt/conda/bin/python3` — в /usr/bin python3 нет pymysql; pymysql есть в conda).
**Осталось (сервер):** материализовать фит в корабль бота — парсить `fitted_item_ids`, спавнить модули как entity-предметы в корабль с правильными flag-слотами (flagHiSlot0=27.., flagMedSlot0=19.., flagLoSlot0=11.., flagRigSlot0=92; EVE_Flags.h), Ship::ModuleManager подхватит их при загрузке. JSON в проекте — есть rapidjson? проверить; иначе простой парсер `[...]`. Распределение по слотам — по категории/атрибуту модуля (high=оружие/турурели, mid=щит/скрам/MWD, low=броня/DCU, rig=риги). Цель: клиент видит реальные модули в фите, при гибели — модули-лут в вреке (как у игрока).
**Плюс (юзер, для полной реалистичности):**
- **Патроны/заряды** в корабль: зарядить оружие фита реальными charges (rocket/light missile/cruise и т.п. в слот заряда оружия; для турелей — патроны). Иначе оружие выглядит пустым.
- **Патроны по скиллам бота**: брать только те charges, которые бот реально может использовать по своим текущим скиллам (skillLevel → требуемые скиллы оружия/заряда; новичок — T1 патроны/ракеты, ветеран — может T2). Согласовать с оружием фита (тип боеприпаса = chargeGroup оружия: rocket/light/heavy/cruise и т.д.).
- **Груз в трюме** по профессии бота (flagCargoHold): майнер возит руду/минералы, раттер/миссионер — лут/сальваж, курьер/трейдер — товар/аммо, хакер — datacores и т.д. Груз делает бота правдоподобным при досмотре/в вреке после гибели.
...
### ✅ Encounter-сдача ПОДТВЕРЖДЕНА (6 сент., сервер на `220a8fb6`): полный цикл работает
- Юзер: принял encounter-миссию (offer 10, mission 60134) → варп-ссылка сработала (WarpToLocation `220a8fb6`, PyWString) → прилетел на цели (лог «warping to ... target (419883932183,...)») → убил 5 целей → лог «offer 10 cleared (all mission targets dead)» → **сдал благополучно**. Сдача работает!
- Лог целей: NPC заспавнены `Encounter NPC 1-0..1-4` (typeID 33001-33004), `OnMissionTargetKilled` декрементит 5→0, `MarkOfferCleared`.
- Нюанс: «галочка» в dungeon-объективе журнала не появляется сразу — косметика клиента (объектив рисует completionStatus при переоткрытии); на сдачу не влияет.
- ⚠️ Замечание юзера: спавненные цели (33001-33103 Guristas Scout/Raider/Warlord/Baron, groupID 25/26/27/419) клиент НЕ показывает как враждебных (не красные, не лочатся в прицел) — это кастомные типы-заглушки. Для «настоящих» красных целей нужно спавнить реальные рат-типы (Asteroid Guristas Frigate 562 = 2382 и т.п.) — проверить/заменить в EncounterServer GetNPCsForFaction.
