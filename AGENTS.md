...
## Current State
Session saved. Server on remote host `172.20.1.47`, SSH user: `dmitry` (password `gbnjy78`), path: `/opt/evemu`. Всё ниже задеплоено (юзер собирает сам). Коммиты в origin/master: `abdeb8fe`...`4a06b77d` (см. секции ниже).
...
- **Проверить после пересборки на `4a06b77d`**: Rogue Drone видны/красные у бельтов и бьют с лучами/цифрами урона; бой в одном баббле (дроны не разбегаются за юзером); поведение ботов в системах без станции и в аномалиях; клиентский десинхрон варпа (разовое, «пока оставим»).
