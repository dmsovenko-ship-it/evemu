...
- **Известное НЕисправленное (нужен рефактор)**: `MissionOffer::bookmarks` — raw PyList*, шарится копиями структур (по-копийный free = double-free); WormholeMgr::Collapse — течёт 1 мелкий SE wrapper на коллапс. `CachedObjectMgr` — дети репы текут (PyRep без clear, принятый класс).
