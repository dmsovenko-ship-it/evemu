...
## Current State
Session saved. Server on remote host `172.20.1.47`, SSH user: `dmitry` (password `gbnjy78`), path: `/opt/evemu`. Всё ниже задеплоено и ПЕРЕСОБРАНО на сервере (docker build, образ `evemu_server:latest` собран, контейнер НЕ пересоздан — ждёт рестарта). Коммиты в origin/master: `abdeb8fe`...`4dfe8700` (см. секции ниже).
...
- Крупные системы (не реализованы): Memory Mgmt/RefPtr→shared_ptr (~400 файлов), PyRep leak-фиксы. Джамп-драйвы/бриджи — РЕШЕНО `6dd25d3d` (валидация активного цино + миграция топлива по расам). Клоны — РЕШЕНО `015f5736`+`ffc27824` (AcceptShipCloneInstallation сломанный SQL + CloneJump не двигал игрока). PI — РЕАЛИЗОВАНО (Colony/PlanetMgr/PlanetDB, PIEnabled=true), нужен e2e-тест. FW Plex capture — РЕАЛИЗОВАНО `b62e1310` (плексы спавнятся, таймеры захвата с лимитами по кораблю, contested-пауза, NPC-защитники, LP, переворот системы по occupierID).
- Выключить диагностику в log.ini после финальной проверки (SE__SLIMITEM/NPC__AI_TRACE/BUBBLE_TRACE/BALL_DECODE).
