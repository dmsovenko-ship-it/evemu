...
## Current State
Session saved. Server on remote host `172.20.1.47`, SSH user: `dmitry` (password `gbnjy78`), path: `/opt/evemu`. Всё ниже задеплоено и ПЕРЕСОБРАНО на сервере (docker build, образ `evemu_server:latest` собран, контейнер НЕ пересоздан — ждёт рестарта). Коммиты в origin/master: `abdeb8fe`...`1be77a41` (см. секции ниже).
