...
## Current State
Session saved. Server on remote host `172.20.1.47`, SSH user: `dmitry` (password `gbnjy78`), path: `/opt/evemu`. Всё ниже задеплоено (юзер собирает сам). Коммиты в origin/master: `abdeb8fe`...`1d1357cd` (см. секции 17-18 августа).
...
- **Маркет sell-ордер «60 jumps» — НЕ РЕШЕНО (диагностика)**: юзер на станции 60001795 (где предметы в ангаре, flag 4) выставляет sell-ордер → «Your Marketing skill only allows sell orders within 60 jumps», ордер не создаётся. Диагностика: клиентский `marketsvc.CanTradeAtStation` для станции юзера возвращает -1 (должен пропускать), серверный чек `orderRange>=0 && orderRange>maxSellRange` (Marketing 5 → 60) в логе НЕ срабатывал (нет ошибки в eve-server.log). Лог-файлы: сервер пишет в `/app/logs/14-08-2026-17-24.log` и `/app/logs/eve-server.log` (MARKET__DUMP в eve-server.log). `Mkt::PlaceCharOrder` видно (2 вызова sell), но dump аргументов отсутствовал → в `fed99ac1` добавлен `call.Dump(MARKET__DUMP)` + лог `orderRange/maxSellRange/marketingLevel` перед чеком. ТРЕБУЕТ пересборки и повторной попытки продажи, чтобы увидеть реальный orderRange. Клиентские таблицы: `marketsvc.jumpsPerSkillLevel={0:-1,1:0,2:5,3:10,4:20,5:50}` (Marketing 5→50), сервер `GetSkillLimits` `jumpsPerSkillLevel={0,5,10,20,40,80}` (5→80), серверный sell-чек `sellRangeBySkill={5,5,10,20,40,60}` (5→60) — ТРИ РАЗНЫХ значения, возможен рассинхрон.
